...
#The basic scheme of a population run is to start with V1R and full outcross, introduce a small fraction of Vr individual, then sample genotype frequencies.
...
script_path<-"/Users/ivian/Desktop/dikaryotic_V.slim"
...
{for (prez in c(0,0.2,0.4,0.6)) #different levels of prezygotic isolation
{for (s2 in c(0,0.25,0.5,0.75,0.90,0.99)) #different selfing rates
...
rep<-100 #number of replicated simulations (see above in the loop that launches slim)
totgen<-1500. #number of generation...see slim code
samplerate<-20 #genotype frequency sampling rate, see slim code
Arun<-totgen/samplerate
setwd("/Users/ivian/Desktop/SlimOuts")
dat<-read.table("output_RUNS.txt")
colnames(dat)<-c("batch","balancing","prezyg","selfing","rep","gen","V1R","Vr","V1r","VR")
str(dat)
datA<-subset(dat,dat$batch=="A")
summary(datA)
datB<-subset(dat,dat$batch=="B")
summary(datB)
  
#--------------------------------------
#analysis of selfing phase. generations 900-1000, 300 gneration after the introduction of the new genotypes.
#Batch B
selfdataB<-data.frame(batch=double(),balancing=double(),prezyg=double(),selfing=double(),V1R=double(),Vr=double(),V1r=double(),VR=double())
str(selfdataB)
...
{for (i in 0:99) #number of replicated runs
{	
sub<-datB[which(datB$balancing==bal & datB$prezyg==pre & datB$selfing==sel ),]
selfdataB<-rbind(selfdataB,data.frame(batch="B",balancing=bal,prezyg=pre,selfing=sel,rep=i,V1R=mean(sub[(45+(Arun*i)):(50+(Arun*i)),7]),Vr=mean(sub[(45+(Arun*i)):(50+(Arun*i)),8]),V1r=mean(sub[(45+(Arun*i)):(50+(Arun*i)),9]),VR=mean(sub[(45+(Arun*i)):(50+(Arun*i)),10])))
}}}}
summary(selfdataB)
selfdataB$balancing<-as.factor(selfdataB$balancing)


#Batch A
selfdataA<-data.frame(batch=double(),balancing=double(),prezyg=double(),selfing=double(),V1R=double(),Vr=double(),V1r=double(),VR=double())
str(selfdataA)
...
selfdataA<-rbind(selfdataA,data.frame(batch="A",balancing=bal,prezyg=pre,selfing=sel,rep=i,V1R=mean(sub[(45+(Arun*i)):(50+(Arun*i)),7]),Vr=mean(sub[(45+(Arun*i)):(50+(Arun*i)),8]),V1r=mean(sub[(45+(Arun*i)):(50+(Arun*i)),9]),VR=mean(sub[(45+(Arun*i)):(50+(Arun*i)),10])))
}}}}
summary(selfdataA)
selfdataA$balancing<-as.factor(selfdataA$balancing)

#Full data A+B
selfdata<-data.frame(batch=double(),balancing=double(),prezyg=double(),selfing=double(),V1R=double(),Vr=double(),V1r=double(),VR=double())
str(selfdata)
selfdata<-rbind(selfdataA,selfdataB)
selfdata$balancing<-as.factor(selfdata$balancing)

selfdata000<-subset(selfdata,selfdata$self==0)
selfdata025<-subset(selfdata,selfdata$self==0.25)
selfdata050<-subset(selfdata,selfdata$self==0.5)
selfdata075<-subset(selfdata,selfdata$self==0.75)
selfdata090<-subset(selfdata,selfdata$self==0.90)
selfdata099<-subset(selfdata,selfdata$self==0.99)
...
g3<-ggplot(selfdata050, aes(x=prezyg, y=V1R,color="darkseagreen3",group=interaction(prezyg,balancing))) +
      geom_boxplot(fill="darkseagreen3",width=0.02, lwd=0.5,position=position_dodge(0.16),outlier.colour="darkseagreen3", outlier.shape=16,
             outlier.size=1)+
      scale_color_manual(name="Strength of balancing selection",values=c("orange", "chocolate3", "red3","red4"))+
      coord_cartesian(ylim=c(0,1))+
      xlab("Level of prezygotic isolation")+
      ...
      stat_summary(fun.y=mean, geom="point",position=position_dodge(0.16), shape=23, size=4)+
       theme_minimal_hgrid(12)
g3<-g3+geom_boxplot( aes(x=prezyg+0.008,y=Vr,color="#B66DE9",group=interaction(prezyg,balancing)), fill= "#B66DE9",width=0.02,lwd=0.5,position=position_dodge(0.16),outlier.colour="#B66DE9", outlier.shape=16,
             outlier.size=1)
g3<-g3+geom_boxplot( aes(x=prezyg-0.008,y=V1r,color="gray",group=interaction(prezyg,balancing)), fill= "gray", alpha=0.85,width=0.02,lwd=0.5,position=position_dodge(0.16),outlier.colour="gray", outlier.shape=16,
             outlier.size=1)
plot(g3)


g3<-ggplot(selfdata050, aes(x=prezyg, y=V1R,group=interaction(prezyg,balancing))) +
      geom_point(colour="darkseagreen3", lwd=0.5,position=position_dodge(0.16),alpha=0.75)+
           coord_cartesian(ylim=c(0,1))+
           geom_boxplot(colour="darkseagreen3",fill="lightgray",width=0.01, lwd=0.5,position=position_dodge(0.16),alpha=0.2,outlier.shape=NA)+
      xlab("Level of prezygotic isolation")+
      ylab("Haplotype frequency")+
      ggtitle("Selfing rate 50%")+
      stat_summary(fun.y=median, geom="point",fill="darkseagreen3",position=position_dodge(0.16), shape=23, size=4)+
       theme_minimal_hgrid(12)
g3<-g3+geom_point( aes(x=prezyg+0.007,y=Vr,group=interaction(prezyg,balancing)), color= "#B66DE9",lwd=0.5,position=position_dodge(0.16))+
           geom_boxplot(aes(x=prezyg+0.007,y=Vr,group=interaction(prezyg,balancing)),colour="#B66DE9",fill="lightgray",width=0.01, lwd=0.5,position=position_dodge(0.16),alpha=0.2,outlier.shape=NA)+
             stat_summary(aes(x=prezyg+0.007,y=Vr,group=interaction(prezyg,balancing)),fun.y=median, geom="point",fill="#B66DE9",position=position_dodge(0.16), shape=23, size=4)
g3<-g3+geom_point( aes(x=prezyg-0.007,y=V1r,group=interaction(prezyg,balancing)), color= "gray", alpha=0.85,lwd=0.5,position=position_dodge(0.16))+
geom_boxplot(aes(x=prezyg-0.007,y=V1r,group=interaction(prezyg,balancing)),colour="gray",fill="lightgray",width=0.01, lwd=0.5,position=position_dodge(0.16),alpha=0.2,outlier.shape=NA)+
stat_summary(aes(x=prezyg-0.007,y=V1r,group=interaction(prezyg,balancing)),fun.y=median,fill="gray", geom="point",position=position_dodge(0.16), shape=23, size=4)
...
g5<-ggplot(selfdata090, aes(x=prezyg, y=V1R,color="darkseagreen3",group=interaction(prezyg,balancing))) +
